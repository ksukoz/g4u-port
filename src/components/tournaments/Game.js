					...
					<Row>
						<Navbar className="transparent z-depth-0">
							<NavItem onClick={this.onClickHandler('compositions')}>
								<div className={`img-wrap ${this.state.compositions ? 'active' : ''}`}>Составы</div>
							</NavItem>
							<NavItem onClick={this.onClickHandler('events')}>
								<div className={`img-wrap ${this.state.events ? 'active' : ''}`}>События</div>
							</NavItem>
							<NavItem onClick={this.onClickHandler('media')}>
								<div className={`img-wrap ${this.state.media ? 'active' : ''}`}>Медиа</div>
							</NavItem>
						</Navbar>
					</Row>
					{this.state.compositions ? <GameComposition id={this.props.match.params.id} /> : ''}
					{this.state.media ? <GameMedia id={this.props.match.params.id} /> : ''}
				</div>
			</section>
		);
	}
...
export default connect(mapStateToProps, { getTourGame })(Game);
