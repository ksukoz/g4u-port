				...
				>
					<div className="container">
						<Row>
							...
							>
								...
								{tournamentsList ? tournamentsList : <option value={0} disabled />}
							</Input>
						</Row>
						<Row>
					...
					<div className="container">
						<Navbar className="transparent">
							<NavItem onClick={(e) => this.onTourNavClick(e, 'main')}>Главная</NavItem>
							<NavItem onClick={(e) => this.onTourNavClick(e, 'news')}>Новости</NavItem>
							<NavItem onClick={(e) => this.onTourNavClick(e, 'table')}>Таблица</NavItem>
							<NavItem onClick={(e) => this.onTourNavClick(e, 'calendar')}>Календарь</NavItem>
							<NavItem onClick={(e) => this.onTourNavClick(e, 'stats')}>Статистика</NavItem>
							<NavItem onClick={(e) => this.onTourNavClick(e, 'results')}>Результаты</NavItem>
							<NavItem onClick={(e) => this.onTourNavClick(e, 'clubs')}>Клубы</NavItem>
							<NavItem onClick={(e) => this.onTourNavClick(e, 'stadiums')}>Поля</NavItem>
							<NavItem onClick={(e) => this.onTourNavClick(e, 'contacts')}>Контакты</NavItem>
						</Navbar>
					</div>
				</div>
				{this.state.main ? <TournamentMain /> : ''}
				{this.state.news ? <TournamentNews id={this.props.match.params.id} /> : ''}
				{this.state.table ? <TournamentTable id={this.props.match.params.id} /> : ''}
				{this.state.calendar ? <TournamentCalendar id={this.props.match.params.id} /> : ''}
				{this.state.stats ? (
					<TournamentStats id={this.props.match.params.id} onClickHandler={this.onClickHandler} />
				) : (
					''
				)}
				{this.state.results ? <TournamentResults id={this.props.match.params.id} /> : ''}
				{this.state.clubs ? <TournamentClubs id={this.props.match.params.id} /> : ''}
				{this.state.stadiums ? <TournamentsStadiums id={this.props.match.params.id} /> : ''}
				{this.state.contacts ? <TournamentContacts id={this.props.match.params.id} /> : ''}
			</section>
		);
	}
}
...
export default connect(mapStateToProps, { getTourInfo })(Tournament);
